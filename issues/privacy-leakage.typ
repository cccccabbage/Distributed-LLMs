...
demonstrated reconstruction from shared gradients, including on a language task. Secure aggregation
can hide individual client updates from the aggregator while it computes their sum, but it does not
protect messages that must remain visible for other protocol stages, such as per-client split-layer
activations.
...
learning. Consequently, moving a cut layer or retaining an initial layer locally should be
understood as reducing direct exposure, not as a formal guarantee.
...
typically trades privacy against utility. Secure computation or trusted execution environments can
reduce what another party learns during computation, but impose their own system assumptions and
costs. These mechanisms address confidentiality risks differently from integrity risks such as
poisoning or backdoors, which require separate defenses.
